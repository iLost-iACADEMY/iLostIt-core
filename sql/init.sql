...
CREATE TABLE `ilost`.`items` (`id` INT NOT NULL AUTO_INCREMENT , `item_name` VARCHAR(1000) NOT NULL , `description` VARCHAR(2999) NULL , `lost_since` TIMESTAMP NOT NULL , `image` VARCHAR(30) NOT NULL COMMENT 'Image Key' , `foundlost_by` INT NOT NULL , `tags` VARCHAR(9999) NOT NULL , `status` VARCHAR(90) NOT NULL , PRIMARY KEY (`id`)) ENGINE = InnoDB;
...
CREATE TABLE `ilost`.`audit` (`id` INT NOT NULL AUTO_INCREMENT , `action` VARCHAR(1000) NOT NULL , `act_by` INT NOT NULL , `description` VARCHAR(1000) NOT NULL , `dedicated_item` INT NULL , `since` TIMESTAMP NOT NULL , PRIMARY KEY (`id`)) ENGINE = InnoDB;
...
ALTER TABLE `audit` ADD CONSTRAINT `fk_deditem` FOREIGN KEY (`dedicated_item`) REFERENCES `items`(`id`) ON DELETE CASCADE ON UPDATE RESTRICT;
