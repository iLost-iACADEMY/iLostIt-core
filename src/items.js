...
router.get('/countitems', async (req, res) => {
    ...
    await tags.forEach(element => {
        const query = `SELECT COUNT(tags) AS counted FROM items LEFT JOIN founded ON items.id = founded.item WHERE items.tags = ?`
...
router.get('/countitems/approved', async (req, res) => {
    con.connect()
    const delay = ms => new Promise(resolve => setTimeout(resolve, ms))
    var tags = [
        "Electronics",
...
router.get('/countitems/pending', async (req, res) => {
    con.connect()
    const delay = ms => new Promise(resolve => setTimeout(resolve, ms))
    var tags = [
        "Electronics",
...
router.get('/countitems/founded', async (req, res) => {
    con.connect()
    const delay = ms => new Promise(resolve => setTimeout(resolve, ms))
    var tags = [
        "Electronics",
    ...
    var tagcount = []
    await tags.forEach(element => {
        const query = `SELECT COUNT(tags) AS counted FROM items LEFT JOIN founded ON items.id = founded.item WHERE items.tags = ? AND founded.founded = 1 AND items.status = 'approved'`
        con.query(query, [element], (err, resu) => {
            tagcount.push(resu[0].counted)
        })
    });
